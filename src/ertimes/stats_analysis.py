...
def get_census_data_for_zipcode() -> pd.DataFrame | None:
    """
    Fetch California median household income data by zip code from the Census API.
    """
    ...
    try:
        ...
        df["county"] = df["name"].str.extract(
            r",\\s*([A-Za-z\\s]+)\\s+County,\\s+California",
            expand=False,
        )
...
def save_to_csv(
    df: pd.DataFrame,
    output_path: str = "data/california_median_income_by_zipcode.csv",
) -> None:
    """
...
def get_income_statistics(
    ...
    """
    Calculate median income statistics overall or by county/city.
    """
    if group_by:
